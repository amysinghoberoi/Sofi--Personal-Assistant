...
if cl.classify(response)== "wquery":
    ...
    d="http://api.ipstack.com/#ip ?access_key=# your access key"
    #use ip stack for gelocation detection
...
#print(a['latitude'],a['longitude'])
    ...
    re=a['region_name']
    co=a['region_code']
    owm = pyowm.OWM('owm id #openweathermap.org')
...
############################6666666666666666666666666666666666666############################################  
    ...
    server.sendmail(
            '#your email id ','#email id of reciever',msg
            )
...
#print(a['latitude'],a['longitude'])
    ...
    CLIENT_ID='four square client id'
    CLIENT_SECRET='four square client secret'
    VERSION='# version'
...
#######################################1010101001010101010101010101010101010##################################    
    ...
    time.sleep(2)
    login=driver.find_element_by_name('username').send_keys("your username")
...
################################131313131313131313313131313##################################################
    ...
    if str(m)=="events" or "event":
        url="https://in.bookmyshow.com/#your city#/events"
       ...
       # engine.say('which event do you want to go for?'),engine.runAndWait()    
        ...
        url="https://in.bookmyshow.com/#your city#/movies"
        response=requests.get(url)
        soup=BeautifulSoup(response.text,'lxml')
...
#####################################1414141414141414141414144141########################################    
    ...
    d="http://api.ipstack.com/# ip ?access_key=# access token"
    #you can store this api globally and call this multiple times
...
########################################################161616161616616161166116161616########################################################################################
    ...
    # whstapp on +number join sign-zulu,join wire- light
    
    client=Client(twilio_sid,auth_token)
    
    contact_dir={'anshul':'number','aakash':'number'}
...
###########################################1717171717171717111711717################################   
    ...
    if str(query) in command:
        os.startfile(r"C:\Users\Desktop\datasets\apps\notepad.txt")
    ...
    if str(query) in command:
        os.startfiler(r"C:\Users\Desktop\datasets\apps\excel.xlsx")
        engine.say("Your query has been generated, sir"),engine.runAndWait()
    ...
    if str(query) in command:
        os.startfile(r"C:\Users\Desktop\datasets\apps\ppt.pptx")
        engine.say("Your query has been generated, sir"),engine.runAndWait()

    word_re=r'\b(?:word|Word|Document|document)\b'
    query=re.findall(word_re,command)
    if str(query) in command:
        os.startfile(r"C:\Users\Desktop\datasets\apps\word.docx")
        engine.say("Your query has been generated, sir"),engine.runAndWait()    

    pic_re=r'\b(?:images|Images|picture|pictures)\b'
    query=re.findall(pic_re,command)
    if str(query) in command:
        os.startfile(r"C:\Users\Desktop\datasets\apps\pic1.jpg")
        engine.say("Your query has been generated, sir"),engine.runAndWait()

    song_re=r'\b(?:song|songs|music)\b'
    query=re.findall(song_re,command)
    if str(query) in command:
        os.startfile(r"C:\Users\Desktop\datasets\apps\song1.mp3")
        engine.say("Your query has been generated, sir"),engine.runAndWait()
